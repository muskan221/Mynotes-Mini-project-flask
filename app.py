...
app.config['SECRET_KEY']='0x000001DE7B3E4E50'
db = SQLAlchemy(app)
...
class Users(db.Model):
    ...
    def exists(username)->bool:
        # "select * from users where usename=? limit 1;"
    ...
    if request.method == 'GET':
        ...
        if (username and name and password):
            if(not Users.exists(username)):
                ...
                flash(f" Please fill all the required details", "warning")
                return redirect(url_for("register"))
...
def index():
    ...
    notes = db.session.execute(notes_sql)# returns tuple
    # print(type(notes))
    # for note in notes:
    #      print(note)
